...
def get_randomly_selected_subset(survey_df, objective_size=100, replace=False):
...
## JUST AS AN EXAMPLE
def EB_polarisation_score(graph_nx, Rep_community, Dem_community):
    ...
    return stats.entropy(p, q)[0]

def compute_shortest_path_length(graph_nx):
    """ gets the length of the shortest path for each node pair in the graph

    Args:
        graph_nx (nx.Graph()): Graph

    Returns:
        [type]: numpy array with the length of every node pair in graph
    """
    path = dict(nx.all_pairs_shortest_path_length(graph_nx))
    results_list = []

    for value in path.values():
        for path_length in value.values():
            results_list.append(path_length)
    return np.array(results_list, dtype=int)

def extract_two_communities(g, data_list):
    ...
    return Rep_community, Dem_community
